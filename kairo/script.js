...
modelRetry?.addEventListener('click',()=>{modelRetry.hidden=true;modelInitialized=false;setProgress(0);setStatus('RETRYING');if(modelHint)modelHint.textContent='Retrying 3D loader...';modelViewer?.setAttribute('src',`assets/oni-0907.glb?v=${Date.now()}`);loadModelViewer()});loadModelViewer();
